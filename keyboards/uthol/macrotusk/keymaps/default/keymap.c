...
void matrix_init_user(void) {
  // Initialize RGB lighting
  rgblight_init();
}

void matrix_scan_user(void) {
  // Call the rgblight_task function to handle RGB effects
  rgblight_task();
}

void keyboard_post_init_user(void) {
  // Set the default RGB mode (replace `RGB_MODE_STATIC_LIGHT` with your desired mode)
  rgblight_mode(RGBLIGHT_MODE_RAINBOW_MOOD +3);
}
